...
func getPage(page int, url string, mainC chan<- []extractedJob) {
	...
	pageUrl := url + "%2F&recruitPage=" + strconv.Itoa(page+1)
	fmt.Println(pageUrl)
...
func writeJobs(jobs []extractedJob) {
	...
	for _, job := range jobs {
		jobSlice := []string{"https://www.saramin.co.kr/zf_user/jobs/relay/view?isMypage=no&rec_idx=" + job.id, job.title, job.location, job.salary, job.summary}
